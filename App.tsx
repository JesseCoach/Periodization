...
const App: React.FC = () => {
  const [programs, setPrograms] = useState<TrainingProgram[]>(() => {
    const saved = localStorage.getItem('snc_athlete_programs_v2'); 
    if (saved) return JSON.parse(saved);
    return [];
  });

  const [activeProgramId, setActiveProgramId] = useState<string | null>(() => {
    const saved = localStorage.getItem('snc_active_athlete_id_v2');
    return saved || null;
  });
  ...
  useEffect(() => {
    localStorage.setItem('snc_athlete_programs_v2', JSON.stringify(programs));
    if (activeProgramId) {
      localStorage.setItem('snc_active_athlete_id_v2', activeProgramId);
    }
  ...
  const deleteProgram = (id: string, name: string) => {
    if (!window.confirm(`確定要刪除運動員「${name}」的所有訓練計畫嗎？此動作無法復原。`)) return;
  ...
  const addStrengthExercise = (cycleId: string, weekId: string, dayId: string) => {
    ...
    const newEx: ExerciseEntry = {
      ...
      reps: '10',
      intensity: '75',
      unit: IntensityUnit.PERCENT_1RM,
      rest: '90s',
      notes: '',
      category: 'Squat'
    };
    
    updateDayData(cycleId, weekId, dayId, {
      strengthExercises: [...day.strengthExercises, newEx]
    });
  };
  ...
  const addConditioningExercise = (cycleId: string, weekId: string, dayId: string) => {
    ...
    const newEx: ConditioningEntry = {
      id: crypto.randomUUID(),
      name: '',
      sets: 3,
      reps: '1',
      intensity: 'MAS',
      unit: IntensityUnit.MAS,
      rest: '1:1',
      notes: '',
    ...
    updateDayData(cycleId, weekId, dayId, {
      conditioningExercises: [...day.conditioningExercises, newEx]
    });
  };
  ...
  const toggleCycleCollapse = (cycleId: string) => {
  ...
  const toggleStatsCollapse = (cycleId: string) => {
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-80 bg-white border-r border-slate-200 flex flex-col shrink-0 transition-transform duration-300 ease-in-out transform ${isLeftDrawerOpen ? 'translate-x-0' : '-translate-x-full'} xl:relative xl:translate-x-0`}>
        <div className="p-6 border-b border-slate-100">
          ...
          <button onClick={createNewProgram} className="w-full bg-blue-600 text-white px-4 py-4 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg">+ 新增紀錄表 (New Record)</button>
        </div>
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {programs.map(p => (
            <div key={p.id} onClick={() => { setActiveProgramId(p.id); setLeftDrawerOpen(false); }} className={`group flex items-center justify-between p-4 rounded-2xl cursor-pointer transition-all border ${activeProgramId === p.id ? 'bg-slate-50 border-blue-200 text-slate-900 shadow-sm' : 'hover:bg-slate-50 text-slate-500 border-transparent'}`}>
              ...
              <button 
                onClick={(e) => { e.stopPropagation(); deleteProgram(p.id, p.athleteName); }}
          ...
          <>
            ...
            <main className="flex-1 overflow-y-auto p-4 md:p-10 scroll-smooth pb-32">
              <div className="max-w-7xl mx-auto space-y-12">
                ...
                {activeProgram.cycles.map((cycle) => {
                  ...
                  return (
                    <div key={cycle.id} className="bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-sm transition-all print:border-slate-100">
                      <div 
                        onClick={() => toggleCycleCollapse(cycle.id)}
                        className={`p-6 md:p-8 flex items-center justify-between cursor-pointer border-b border-slate-100 transition-colors ${isCycleCollapsed ? 'bg-slate-50/50' : 'bg-slate-900 text-white print:bg-slate-50 print:text-slate-900'}`}
                      >
                        <div className="flex items-center gap-6">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black no-print ${isCycleCollapsed ? 'bg-slate-200 text-slate-500' : 'bg-blue-600 text-white'}`}>
                            <svg className={`w-5 h-5 transition-transform ${isCycleCollapsed ? '' : 'rotate-180'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7" /></svg>
                          </div>
                          <input 
                            type="text" 
                            value={cycle.name} 
                            onClick={(e) => e.stopPropagation()}
                            onChange={(e) => updateActiveProgram({ cycles: activeProgram.cycles.map(c => c.id === cycle.id ? { ...c, name: e.target.value } : c) })}
                            className="text-xl md:text-2xl font-black bg-transparent focus:outline-none tracking-tight"
                          />
                        </div>
                        <div className="flex items-center gap-3 no-print">
                          {!isCycleCollapsed && (
                            <button onClick={(e) => { e.stopPropagation(); addWeek(cycle.id); }} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest">+ADD WEEK</button>
                          )}
                        </div>
                      </div>
                      ...
                      {!isCycleCollapsed && (
                        <div className="p-6 md:p-8 space-y-10">
                          <div className="bg-slate-50 rounded-[2rem] border border-slate-100 overflow-hidden">
                            <div 
                              onClick={() => toggleStatsCollapse(cycle.id)}
                              className="px-6 py-4 flex items-center justify-between cursor-pointer bg-slate-100/50 hover:bg-slate-100 transition-colors no-print"
                            >
                              <div className="flex items-center gap-3">
                                <svg className="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
                                <span className="text-xs font-black uppercase tracking-widest text-slate-600">Performance Baseline (1RM & MAS)</span>
                              </div>
                            ...
                            <div className={`p-6 grid grid-cols-2 md:grid-cols-7 gap-4 ${(isStatsCollapsed && !window.matchMedia('print').matches) ? 'hidden' : 'grid'}`}>
                                {[
                                  ...
                                  { label: 'Hinge', key: 'deadlift1rm' },
                                  { label: 'Push', key: 'push1rm' },
                                  { label: 'Pull', key: 'pull1rm' },
                                  { label: 'Press', key: 'press1rm' },
                                  { label: 'MAS', key: 'mas' },
                                  { label: 'MSS', key: 'mss' },
                                ].map((stat) => (
                                  <div key={stat.key} className="flex flex-col gap-1.5">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-tight">{stat.label}</label>
                                    <input 
                                      type="number" 
                                      value={cycle.stats[stat.key as keyof AthleteStats] || 0}
                                      onChange={(e) => updateCycleStats(cycle.id, { [stat.key]: parseFloat(e.target.value) || 0 })}
                                      className="bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-sm font-bold text-slate-800 focus:ring-1 focus:ring-blue-500 outline-none" 
                                    />
                                  </div>
                                ))}
                            </div>
                            ...
                            return (
                              <div key={week.id} className="space-y-6">
                                <div 
                                  ...
                                  className="bg-slate-50 p-4 rounded-2xl flex items-center justify-between cursor-pointer group hover:bg-slate-100 transition-all border border-slate-100 print:bg-slate-100"
                                >
                                  <div className="flex items-center gap-4">
                                    <div className="bg-slate-900 text-white w-9 h-9 rounded-lg flex items-center justify-center font-black text-xs">W{week.weekNumber}</div>
                                    <input type="text" value={week.title} onClick={(e) => e.stopPropagation()} onChange={(e) => updateActiveProgram({ cycles: activeProgram.cycles.map(c => c.id === cycle.id ? { ...c, weeks: c.weeks.map(w => w.id === week.id ? { ...w, title: e.target.value } : w) } : c) })} className="bg-transparent text-lg font-black text-slate-800 focus:outline-none" />
                                  </div>
                                    ...
                                    {week.days.map((day) => {
                                      ...
                                      return (
                                        <div key={day.id} onClick={() => setActiveDayId(day.id)} className={`bg-white rounded-[2rem] border transition-all ${activeDayId === day.id ? 'ring-4 ring-blue-500/10 border-blue-300' : 'border-slate-200'}`}>
                                          <div onClick={(e) => { e.stopPropagation(); toggleDayCollapse(day.id); }} className="p-5 flex items-center justify-between cursor-pointer">
                                            <div className="flex items-center gap-3">
                                              <svg className={`w-4 h-4 text-slate-300 transition-transform no-print ${isDayCollapsed ? '' : 'rotate-180'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7" /></svg>
                                              <input type="text" value={day.title} onClick={(e) => e.stopPropagation()} onChange={(e) => updateDayData(cycle.id, week.id, day.id, { title: e.target.value })} className="font-black text-slate-800 bg-transparent text-lg focus:outline-none" />
                                            </div>
                                          </div>
                                          ...
                                          {!isDayCollapsed && (
                                            <div className="px-6 md:px-8 pb-8 space-y-8">
                                              <div className="pt-2">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Warm-up / Activation</label>
                                                <textarea value={day.warmup || ''} onChange={(e) => updateDayData(cycle.id, week.id, day.id, { warmup: e.target.value })} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm focus:ring-1 focus:ring-blue-500 transition-all outline-none resize-none" placeholder="Daily activation routine..." rows={2} />
                                              </div>
                                              
                                              <section>
                                                <div className="flex items-center justify-between mb-4">
                                                  <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Strength Exercises</h3>
                                                  <button onClick={() => addStrengthExercise(cycle.id, week.id, day.id)} className="text-blue-600 text-[10px] font-black uppercase tracking-widest hover:underline no-print">+ Add Exercise</button>
                                                </div>
                                                <div className="space-y-3">
                                                  {day.strengthExercises.map(ex => (
                                                    <ExerciseRow key={ex.id} exercise={ex} athleteStats={cycle.stats} onUpdate={(u) => {
                                                      const updatedExs = day.strengthExercises.map(e => e.id === ex.id ? { ...e, ...u } : e);
                                                      updateDayData(cycle.id, week.id, day.id, { strengthExercises: updatedExs });
                                                    }} onDelete={() => {
                                                      updateDayData(cycle.id, week.id, day.id, { strengthExercises: day.strengthExercises.filter(e => e.id !== ex.id) });
                                                    }} />
                                                  ))}
                                                </div>
                                              </section>
                                              ...
                                              <section>
                                                <div className="flex items-center justify-between mb-4">
                                                  <h3 className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Conditioning Drills</h3>
                                                  <button onClick={() => addConditioningExercise(cycle.id, week.id, day.id)} className="text-blue-600 text-[10px] font-black uppercase tracking-widest hover:underline no-print">+ Add Drill</button>
                                                </div>
                                                <div className="space-y-4">
                                                  {day.conditioningExercises.map(ex => (
                                                    <ConditioningRow key={ex.id} exercise={ex} athleteStats={cycle.stats} onUpdate={(u) => {
                                                      const updatedExs = day.conditioningExercises.map(e => e.id === ex.id ? { ...e, ...u } : e);
                                                      updateDayData(cycle.id, week.id, day.id, { conditioningExercises: updatedExs });
                                                    }} onDelete={() => {
                                                      updateDayData(cycle.id, week.id, day.id, { conditioningExercises: day.conditioningExercises.filter(e => e.id !== ex.id) });
                                                    }} />
                                                  ))}
                                                </div>
                                              </section>
                ...
                <button onClick={addCycle} className="w-full py-10 border-4 border-dashed border-slate-200 rounded-[3rem] flex flex-col items-center justify-center gap-3 hover:border-blue-400 group transition-all no-print">
                  ...
                  <span className="text-sm font-black text-slate-400 uppercase tracking-[0.2em] group-hover:text-blue-600">Start New Training Cycle</span>
                </button>
              </div>
            </main>
          </>
          ...
          <div className="space-y-12">
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
              <div className="flex flex-col gap-1 mb-6">
                <h3 className="font-black text-xl flex items-center gap-3"><span className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>Fatigue Monitoring</h3>
                {activeDay && <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest ml-5">Logged: C{activeDay.cycleNumber}W{activeDay.weekNumber}D{activeDay.dayNumber}</span>}
              </div>
                ...
                <div className="space-y-6 relative z-10">
                  <div className="bg-white/5 p-5 rounded-2xl border border-white/10">
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Soreness / Readiness</span>
                      <span className="text-xl font-black">{activeDay.day.fatigueIntensity || 5}</span>
                    </div>
                    <input type="range" min="1" max="10" step="1" value={activeDay.day.fatigueIntensity || 5} onChange={(e) => updateDayData(activeDay.cycleId, activeDay.weekId, activeDay.day.id, { fatigueIntensity: parseInt(e.target.value) })} className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                  </div>
                  
                  <div className="space-y-2">
                    {FATIGUE_ITEMS.map((item) => (
                      <div key={item.key} onClick={() => {
                        const current = activeDay.day.fatigueChecks || {};
                        updateDayData(activeDay.cycleId, activeDay.weekId, activeDay.day.id, { fatigueChecks: { ...current, [item.key]: !current[item.key] } });
                      }} className={`flex items-center justify-between p-4 rounded-2xl cursor-pointer border transition-all active:scale-95 ${activeDay.day.fatigueChecks?.[item.key] ? 'bg-blue-600 border-blue-400 text-white' : 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10'}`}>
                        <div className="flex items-center gap-3"><span className="text-lg">{item.icon}</span><span className="text-xs font-black uppercase tracking-widest">{item.label}</span></div>
                        {activeDay.day.fatigueChecks?.[item.key] && <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>}
                      </div>
                    ))}
                  </div>

                  <div className="bg-white/5 p-5 rounded-2xl border border-white/10">
                    <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest block mb-2">Session Notes</label>
                    <textarea value={activeDay.day.fatigueNotes || ''} onChange={(e) => updateDayData(activeDay.cycleId, activeDay.weekId, activeDay.day.id, { fatigueNotes: e.target.value })} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-200 focus:ring-1 focus:ring-blue-500 transition-all outline-none resize-none" placeholder="Recovery notes, specific injury markers..." rows={4} />
                  </div>
                </div>
              ) : (
